...
Tg1 = getTransforms_iiwa7_eef_mex(qg1);
Tg2 = getTransforms_iiwa7_eef_mex(qg2);
Tg3 = getTransforms_iiwa7_eef_mex(qg3);

ng = 3;
Tg = repmat(Tg3, [1, 1, Nmax_sim]);
Tg(:, :, 1 : floor(Nmax_sim/ng)) = repmat(Tg1, [1, 1, floor(Nmax_sim/ng)]);
Tg(:, :, ceil(Nmax_sim/ng) : floor(2*Nmax_sim/ng)) = repmat(Tg2, [1, 1, floor(2*Nmax_sim/ng)-ceil(Nmax_sim/ng)+1]);
...
hold off
grid on
title('Switching Goal Pose Tracking Error $\texttt{plog}({}^gT_k)$ via Direct Pose Tracking', 'Interpreter', 'latex')
...
f = gcf
exportgraphics(f,'figures/boxcon_MPC_x6_simtime_switching.png','Resolution',500)
