...
title('Reference position trajectory (constant orientation)', 'Interpreter', 'latex')
set(gca,'TickLabelInterpreter','latex')
f = gcf
exportgraphics(f,'figures/reference_trajectory.png','Resolution',500)
...
% Tg = [Rg, pg(:,1); 0, 0, 0 1];
% 
% [q0, solnInfo0] = ik(eef, Tg, [1 1 1 1 1 1], zeros(7,1));

% q0 = q0 + [-0.15; 0; 0; 0; 0; 0; 0];
q0 = zeros(7,1);
...
for i = 1:Nmax_sim-1
    ...
    if (mpcTimeFactor==1) || (mod(i, mpcTimeFactor) == 1)   % do MPC at i = 1, 11, 21, ...
        ...
        % Inverse kinematics
        Tg = [Rg, pg(:,i); 0, 0, 0, 1];
        [qg(:,k), solnInfo(k)] = ik(eef, Tg, [1 1 1 1 1 1], q(:,i));
        f = f0 * (q(:,i) - qg(:,k));
...
hold off
grid on
title('Pose Reference Tracking Error $\texttt{plog}({}^gT_k)$ via Joint Tracking and IK ', 'Interpreter', 'latex')
...
f = gcf
exportgraphics(f,'figures/boxcon_MPC_q_simtime_following.png','Resolution',500)
